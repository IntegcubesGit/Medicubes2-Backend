    ...
    public class JWTService : IJWTService
    {
        ...
        public string GenerateToken(AppUser app, IConfiguration config, out DateTime expiry)
        {
            ...
            var claims = new[]
            {
                ...
                new Claim("TenantId", app.TenantId.ToString())
            };
        ...
        public TempUser DecodeToken()
        {
            ...
            var user = new TempUser
            {
                ...
                IsSuperAdmin = Convert.ToInt32(jwtToken.Claims.FirstOrDefault(c => c.Type == "IsSuperAdmin")?.Value),
                TenantId = Convert.ToInt32(jwtToken.Claims.FirstOrDefault(c => c.Type == "TenantId")?.Value)
            };
