...
namespace Infrastructure.Repositories
{
    public class UserService : IUserService
    {
        ...
        public UserService(IJWTService jwtService, AppDbContext db, ITimeService service, AuthDbContext authdb, UserManager<AppUser> _userManager)
        {
        ...
        public async Task<object> RegisterUser(RegisterOrUpdateUserRequestDTO registerUserRequest, List<int> roles)
        {
            ...
            var tenantId = jwtClaims.TenantId;
        ...
        {
            //var jwtClaims = jWTService.DecodeJWTToken();

            var rawData = await (from users in _db.Users.Where(u => u.IsDeleted == 0)
        ...
        public async Task<object> GetUserById(int userId)
        {
            ...
            var rawData = await (from _user in _db.Users
                                 where _user.IsDeleted == 0 && _user.TenantId == 1 && _user.Id == userId
                                 select new
                                 {
                                     _user.Id,
                                     FullName = _user.Name,
            ...
            var roles = await (from userRole in _db.UserRoles.Where(x => x.UserId == userId && x.IsDeleted == 0 && x.TenantId == 1)
        ...
        public async Task<object> GetUserInfoById(int userId)
        {
            ...
            var user = await _db.Users.Where(u => u.Id == userId && u.TenantId == tenantId && u.IsDeleted == 0).FirstOrDefaultAsync();
        ...
        public async Task<object> GetOrgLocationsById(int userId)
        {
            ...
            var panelDetails = await (from location in _db.OrgLocations.Where(l => l.StatusId == 1 && l.TenantId == jwtClaims.TenantId)
