...
namespace Infrastructure.Repositories
{
    public class OrgAppSettingService : IOrgAppSettingService
    {
        ...
        public OrgAppSettingService(IJWTService jWTService, AppDbContext appDbContext)
        {
        ...
        {
            var user = _jWTService.DecodeToken();

            var existing = await _db.OrgAppSettings.FirstOrDefaultAsync(x => x.TenantId == user.TenantId && x.IsDeleted == 0);
        ...
        public async Task<object> GetInvoiceSetting()
        {
            var tempUser = _jWTService.DecodeToken();
            var res = await _db.OrgAppSettings.Where(a => a.IsDeleted == 0 && a.TenantId == tempUser.TenantId).Select(a => a.InvoiceColor).FirstOrDefaultAsync();
            if(res == null)
            {
                return new
                {
        ...
        {
            var tempUser = _jWTService.DecodeToken();
            var res = await (from org in _db.OrgAppSettings.Where(a => a.IsDeleted == 0 && a.TenantId == tempUser.TenantId)
                             join curr in _db.ICurrencies.Where(a => a.IsDeleted == 0) on org.CurrencyId equals curr.CurrencyId
                             join state in _db.IStateTypes.Where(a => a.IsDeleted == 0) on org.StateId equals state.StateId into stateGroup
